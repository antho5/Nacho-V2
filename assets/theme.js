  ...
  var halo = {
              ...
              $.ajax({
                ...
                success: function (data) {
                  if (url != "") {
                    ...
                    $block
                      .find(
                        ".card .variants-popup-content .selector-wrapper .swatch-element"
                      )
                      .each(function () {
          ...
          if (entry.isIntersecting) {
            ...
            tabLink.off("click").on("click", function (e) {
              ...
              if (!$(this).hasClass("active")) {
                $(".list-tabs-popup.show-dropdown")?.removeClass(
                  "show-dropdown"
                );
                $(this)
                  .closest(".dropdown")
                  .find(".label-active .tab-links")
                  .text($(this).text());
            ...
            $(".product-tab-block .dropdown .label-active .list-product-tabs")
              .off("click")
              .on("click", function (e) {
                document
                  .querySelector(".list-tabs-popup")
        ...
        if (productGrid.not(".slick-initialized")) {
          ...
          if (productGrid.hasClass("enable_progress_bar")) {
            ...
            productGrid.on("init", (event, slick) => {
              var percent =
                (slick.currentSlide / (slick.slideCount - itemToShow)) * 100 +
                "%";
          ...
          if (productGrid.hasClass("enable_progress_bar")) {
            productGrid.on("afterChange", (event, slick, nextSlide) => {
              var percent =
                (nextSlide / (slick.slideCount - itemToShow)) * 100 + "%";
              progressBar.css("--percent", percent);
            });
          }
                ...
                if (window.quick_cart.show) {
                  Shopify.getCart((cart) => {
                    ...
                    if (cart.item_count >= 100) {
                      $body
                        .find(".cart-count-bubble [data-cart-count]")
    ...
    actionAddToCart2: function ($target, productForm) {
      ...
      const properties = document.querySelectorAll(
        '.line-item-property__field [name^="properties"]'
      );
    ...
    initVariantImageGroup: function ($scope, enable = false) {
      if (enable) {
        ...
        $doc.on("change", "input[data-filter]", (event) => {
          ...
          console.log("thisMetafields", thisMetafields);
    ...
    typingAnimation() {
      ...
      MultipleSentencesType.prototype.setupCurrentSentence = function () {
        this.currentTextArray = this.sentences[this.sentenceIndex]
          .split(",")
          .map((word) => word.trim());
      ...
      MultipleSentencesType.prototype.tick = function () {
        ...
        if (!this.isDeleting && !this.isPaused) {
          for (let i = 0; i < this.currentTextArray.length; i++) {
            if (this.displayedWords[i] !== completedTextArray[i]) {
              this.displayedWords[i] = completedTextArray[i].substring(
                0,
                this.displayedWords[i].length + 1
              );
              break;
            }
          }
          ...
          for (let i = this.currentTextArray.length - 1; i >= 0; i--) {
            if (this.displayedWords[i] !== "") {
              this.displayedWords[i] = this.displayedWords[i].substring(
                0,
                this.displayedWords[i].length - 1
              );
              break;
        ...
        var spans = this.displayedWords
          .map((word) => `<span class="text">${word}</span>`)
          .join(" ");
