class VariantSelects extends HTMLElement {
  constructor() {
    ...
    this.isFullWidth =
      this.item.closest(".product-full-width").length == 1 ||
      this.item.closest(".product-full-width-2").length == 1;
  ...
  updateMedia(time, status) {
    const enableVariantImageGroup = document?.querySelector(
      ".enable_variant_image_group"
    );
    ...
    setTimeout(() => {
      ...
      const fallbackImageContainer = document?.querySelector(
      ...
      if (activeMedia) {
        ...
        setTimeout(() => {
          fallbackImageContainer.scrollIntoView({
            behavior: "smooth",
            block: "start",
          });
        }, 20);
  ...
  scrollToBlock(block) {
    const headerHeight = document
      .querySelectorAll(".section-header-navigation")[0]
      ?.getBoundingClientRect().height;
    const announcementBarHeight = document
      .querySelectorAll(".announcement-bar")[0]
      ?.getBoundingClientRect().height;
    const positionTop =
      block.getBoundingClientRect().top - headerHeight - announcementBarHeight;
  ...
  updateAttribute(unavailable = true, disable = true) {
    ...
    if (isNaN(maxValue)) {
      maxValue = maxValue = parseInt(
        buttonAddtocart.attr("data-inventory-quantity")
      );
    } else {
      maxValue = parseInt(quantityInput.attr("data-inventory-quantity"));
    }
    ...
    if (unavailable) {
      ...
      if (disable) {
        ...
        const stickyPrice = $(
          "[data-sticky-add-to-cart] .money-subtotal .money"
        );
        ...
        if (
          subTotal != 0 &&
          stickyComparePrice.length &&
          window.subtotal.show
        ) {
          let comparePrice = $(
              "[data-sticky-add-to-cart] .money-compare-price"
            ).data("compare-price"),
        ...
        if (compare_at_price == current_at_price) {
          ...
          if (compare_at_price) {
            ...
            if (thisComparePrice.length) {
              ...
              const comparePrice_sticky = extractContent(
                compare_at_price_sticky
              );
  ...
  checkQuantityWhenVariantChange() {
    ...
    const elementProductAddToCart = document.getElementById(
      "product-add-to-cart"
    );
    if (
      elementProductAddToCart &&
      this.currentVariant?.available !== undefined &&
  ...
  updateVariantStatuses() {
    ...
    inputWrappers.forEach((option, index) => {
      option.querySelector("[data-header-option]").innerText =
        option.querySelector(":checked").value;
  ...
  updateBadgeSale() {
    ...
    let badgeText = badgeWrap.querySelector(
        ".productView-badge .badge.sale-badge"
      ),
      badgeTextSaleType = badgeWrap.getAttribute("data-text-sale-badge"),
      priceCompare = document
        .querySelector(".productView-product .price__compare")
        .getAttribute("data-compare"),
      priceLast = document
        .querySelector(".productView-product .price__sale .price__last")
        .getAttribute("data-last"),
      percentSale = Math.round(
        ((priceCompare - priceLast) / priceCompare) * 100
      );
      ...
      if (availableOptionInputsValue.includes(input.getAttribute("value"))) {
        ...
        if (
          ...
          input
            ...
            .querySelector(`input[value="${inputsValue[0]}"]`).checked = true;
...
class QuantityInput extends HTMLElement {
  ...
  initProductQuantity() {
    if (this.inven_array != undefined) {
      ...
      this.querySelector('input[name="quantity"]').dataset.inventoryQuantity =
        inventoryQuantity;
    }
  }
